            ...
            {m.toolInvocations?.map((toolInvocation: ToolInvocation) => {
              ...
              if (toolInvocation.toolName === 'shopForProduct') {
                ...
                if ('result' in toolInvocation) {
                  ...
                  console.log("toolInvocation.result.shopping_results")
                  console.log(toolInvocation.result.shopping_results)
                  console.log("toolInvocation.result.shopping_results done")

                  console.log("toolInvocation.result")
                  console.log(toolInvocation.result)
                  console.log("toolInvocation.result end")

                  return (
                    <View key={toolCallId} style={styles.toolInvocationContainer}>
                      <HorizontalProducts
                        products={toolInvocation.result.shopping_results}
                        onProductPress={(selectedProductId, allProducts) => {
                          router.push({
                            pathname: '/(tabs)/product-details',
                            params: { 
                              selectedProductId,
                              productsList: JSON.stringify(allProducts)
                            },
                          });
                        }}
                      />
